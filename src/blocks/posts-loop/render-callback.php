<?php
/**
 * Renders the posts loop block content
 *
...
/**
 * Render posts loop block content
 *
 * @param array $attributes List of block attributes defined in the block.json file.
 * @return string HTML markup for the block.
 */
function caledros_basic_blocks_posts_loop_render_cb( $attributes ) {
	// Placeholder images.
	$placeholder_background_url = plugins_url( 'build/images/placeholder-mountains.7da2eb2d.webp', dirname( dirname( __DIR__ ) ) );
	if ( ! filter_var( $placeholder_background_url, FILTER_VALIDATE_URL ) ) {
		$placeholder_background_url = '';
	}
	$placeholder_url = plugins_url( 'build/images/placeholder.06efdda2.webp', dirname( dirname( __DIR__ ) ) );
	...
	$selected_date_format = $date_format_map[ $attributes['dateOptions']['dateFormat'] ] ?? 'DD/MM/YYYY';

	// Page type attribute.
	$page_type_query = 'front-page' === $page_type ? 'page' : 'paged';
	...
	?>     
		<?php
			...
			if ( 'search-results-page' === $page_type ) {
				$search_string = get_search_query();
				$args['s']     = $search_string;
			}
			if ( 'category-template' === $page_type && is_category() ) {
			...
			if ( $tag_filter_enabled && 'tag-template' !== $page_type ) {
				$args['tag__in'] = array( $tag_id );
			}
			if ( 'author-template' === $page_type && is_author() ) {
				$current_author = get_queried_object();
				$author_id      = $current_author->ID;
				$args['author'] = $author_id;
			}
			if ( $author_filter_enabled && 'author-template' !== $page_type ) {
		...
		<?php if ( ! $show_demo_data ) : ?>
			<?php if ( 'search-results-page' === $page_type ) : ?>
					...
					<div class="cbb-posts-loop__card">  
						...
						<div class="<?php echo wp_kses_post( $post_info_classes ); ?>">
							<div class="cbb-posts-loop__category-and-tag">  
								...
								<?php endif; ?>  
								<?php
								if ( ( ( 'tag-template' === $page_type && is_tag() ) || ( $tag_filter_enabled && 'tag-template' !== $page_type ) ) && $show_tags ) :
