...
export default function IconColorSettings({ attributes, setAttributes }) {
  const { buttonIconColor } = attributes;
  ...
  return (
    <PanelBody
      title={__("Icon color (light mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ColorPalette
        colors={editorPalette ? colorOptions : fallbackColorOptions}
        value={buttonIconColor}
        onChange={(newColor) => setAttributes({ buttonIconColor: newColor })}
        enableAlpha={true}
        clearable={false}
      />
    </PanelBody>
