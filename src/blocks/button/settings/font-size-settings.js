...
export default function FontSizeSettings({ attributes, setAttributes }) {
  const { buttonFontSize } = attributes;
  ...
  const [useCustomFontSize, setUseCustomFontSize] = useState(
    registeredFontPresets().includes(buttonFontSize) ? false : true
  );
    ...
    >
      ...
      {!useCustomFontSize && (
        <SelectControl
          ...
          value={
            registeredFontPresets().includes(buttonFontSize)
              ? buttonFontSize
              : ""
          }
          options={[
      ...
      {useCustomFontSize && (
        <div className="cbb-editor__grid">
          <RangeControl
            ...
            onChange={(newFontSize) => {
              setAttributes({
                buttonFontSize: `${enforceMaxValue(
                  fontSizeUnit,
                  newFontSize
                )}${fontSizeUnit}`,
              });
