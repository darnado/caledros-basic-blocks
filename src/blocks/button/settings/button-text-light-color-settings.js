...
export default function ButtonTextLightColorSettings({
  attributes,
  setAttributes,
}) {
  const { buttonTextLightColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonTextLightColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Text color (light mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonTextLightColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ buttonTextLightColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
