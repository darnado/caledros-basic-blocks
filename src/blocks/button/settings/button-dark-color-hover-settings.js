...
export default function ButtonDarkColorHoverSettings( {
	attributes,
	setAttributes,
} ) {
	const { buttonHoverDarkColor } = attributes;
	const [ useTransparentBackground, setUseTransparentBackground ] = useState(
		buttonHoverDarkColor === '#00000000' ? true : false
	);
		...
		>
			<ToggleControl
				...
				onChange={ () => {
					...
					setAttributes( { buttonHoverDarkColor: '#00000000' } );
				} }
			/>
			{ ! useTransparentBackground && (
				<ColorPalette
					...
					onChange={ ( newColor ) =>
						setAttributes( { buttonHoverDarkColor: newColor } )
					}
					enableAlpha={ true }
					clearable={ false }
				/>
