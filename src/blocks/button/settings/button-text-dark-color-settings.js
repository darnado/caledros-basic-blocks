...
export default function ButtonTextDarkColorSettings({
  attributes,
  setAttributes,
}) {
  const { buttonTextDarkColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonTextDarkColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Text color (dark mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonTextDarkColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ buttonTextDarkColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
