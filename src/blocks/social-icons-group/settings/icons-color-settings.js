...
export default function IconsColorSettings({ attributes, setAttributes }) {
  const { socialIconsColor } = attributes;
  ...
  return (
    <PanelBody
      title={__("Social icons color (light mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ColorPalette
        colors={editorPalette ? colorOptions : fallbackColorOptions}
        value={socialIconsColor}
        onChange={(newColor) => setAttributes({ socialIconsColor: newColor })}
        enableAlpha={true}
        clearable={false}
      />
    </PanelBody>
