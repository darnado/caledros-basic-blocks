...
export default function IconsDarkColorSettings({ attributes, setAttributes }) {
  const { socialIconsDarkColor } = attributes;
  ...
  return (
    <PanelBody
      title={__("Social icons color (dark mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ColorPalette
        ...
        onChange={(newColor) =>
          setAttributes({ socialIconsDarkColor: newColor })
        }
        enableAlpha={true}
        clearable={false}
      />
