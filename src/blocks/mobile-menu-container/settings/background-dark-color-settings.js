...
export default function BackgroundDarkColorSettings({
  attributes,
  setAttributes,
}) {
  const { containerDarkBackgroundColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    containerDarkBackgroundColor === "#00000000" ? true : false
  );
  ...
  return (
    ...
    >
      <ToggleControl
        ...
        label={__("Use transparent background.", "caledros-basic-blocks")}
        checked={useTransparentBackground}
        onChange={() => {
          setUseTransparentBackground((oldValue) => !oldValue);
          setAttributes({ containerDarkBackgroundColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ containerDarkBackgroundColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
