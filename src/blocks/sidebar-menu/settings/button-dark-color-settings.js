...
export default function ButtonDarkColorSettings({ attributes, setAttributes }) {
  // Block attributes
  const { buttonDarkColor } = attributes;

  // Set state for the use of transparent color
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonDarkColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Button color (dark mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonDarkColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) => setAttributes({ buttonDarkColor: newColor })}
          enableAlpha={true}
          clearable={false}
        />
      )}
