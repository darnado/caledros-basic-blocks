...
export default function BackgroundDarkColorSettings({
  ...
  const { darkBackgroundColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    darkBackgroundColor === "#00000000" ? true : false
  );
    ...
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ darkBackgroundColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ darkBackgroundColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
