...
export default function CardTwoTemplateSettings({ attributes, setAttributes }) {
  const { cardTwoSlug } = attributes;
  ...
  return (
    <PanelBody
      title={__("2nd Card Template", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ComboboxControl
        ...
        onChange={(slugValue) => {
          setAttributes({ cardTwoSlug: slugValue });
        }}
      />
    </PanelBody>
  );
