<?php
/**
 * Renders the content of the category loop block
 *
...
/**
 * Render content of the category loop block
 *
 * @param array $attributes Block attributes declared in the block.json file.
		...
		<div class="cbb-categories-loop" <?php echo wp_kses_post( $inline_styles ); ?>>
			<div class="cbb-categories-loop__container">
				...
				foreach ( $categories as $category ) {
					if ( ! $show_uncategorized_category && ( 'uncategorized' === $category->slug || 1 === $category->term_id ) ) {
						continue;
					}
					...
					$meta_alt_text   = get_term_meta( $category_id, 'alt_text_category_image', true );
					$alt_text        = $meta_alt_text ? $meta_alt_text : 'placeholder';

					$text_number_articles = 1 === $number_articles
						? __( 'publication', 'caledros-basic-blocks' )
						: __( 'publications', 'caledros-basic-blocks' );

					$category_loop_title_image_url = $categories_loop_title_icon_url ? $categories_loop_title_icon_url : $placeholder_url;
					?>
