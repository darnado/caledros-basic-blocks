...
export default function LightColorSettings({ attributes, setAttributes }) {
  const { lightColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    lightColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__(
        "Navigation & Pagination Color (Light Mode)",
    ...
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ lightColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          value={lightColor}
          onChange={(newColor) => setAttributes({ lightColor: newColor })}
          enableAlpha={true}
          clearable={false}
        />
      )}
