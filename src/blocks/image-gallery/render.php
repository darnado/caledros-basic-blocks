<?php
/**
 * Renders the content of the image gallery block
 *
...
// Demo images.
$caledros_basic_blocks_bird_image_url = plugins_url( 'build/images/bird.8dbbb5a8.webp', dirname( dirname( __DIR__ ) ) );
if ( ! filter_var( $caledros_basic_blocks_bird_image_url, FILTER_VALIDATE_URL ) ) {
	$caledros_basic_blocks_bird_image_url = '';
}
$caledros_basic_blocks_flower_image_url = plugins_url( 'build/images/flower.1092b3c8.webp', dirname( dirname( __DIR__ ) ) );
if ( ! filter_var( $caledros_basic_blocks_flower_image_url, FILTER_VALIDATE_URL ) ) {
	$caledros_basic_blocks_flower_image_url = '';
}
$caledros_basic_blocks_mountains_image_url = plugins_url( 'build/images/mountains.2b50cd4a.webp', dirname( dirname( __DIR__ ) ) );
if ( ! filter_var( $caledros_basic_blocks_mountains_image_url, FILTER_VALIDATE_URL ) ) {
	$caledros_basic_blocks_mountains_image_url = '';
}

// BLOCK ATTRIBUTES.
$caledros_basic_blocks_images                   = $attributes['images'] ?? array();
...
$caledros_basic_blocks_wp_context = array(
...
$caledros_basic_blocks_wp_context_json = htmlspecialchars( wp_json_encode( $caledros_basic_blocks_wp_context ), ENT_QUOTES, 'UTF-8' );
		...
		if ( ! $caledros_basic_blocks_show_demo_data ) {
			...
			foreach ( $caledros_basic_blocks_images as $caledros_basic_blocks_image ) {
				...
				echo wp_get_attachment_image(
					$caledros_basic_blocks_image_id,
					'full',
					false,
					array(
						'alt'   => esc_attr( $caledros_basic_blocks_image_alt ),
						'style' => esc_attr( $caledros_basic_blocks_img_style ),
					)
				);
				echo '</div>';
			}
