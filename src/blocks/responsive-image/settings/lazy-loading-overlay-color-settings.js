...
export default function LazyLoadingOverlayColorSettings({
  attributes,
  setAttributes,
}) {
  const { lazyLoadingOverlayColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    lazyLoadingOverlayColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Lazy loading overlay color", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        __nextHasNoMarginBottom
        label={__("Use a transparent overlay", "caledros-basic-blocks")}
        checked={useTransparentBackground}
        onChange={() => {
          setUseTransparentBackground((oldValue) => !oldValue);
          setAttributes({ lazyLoadingOverlayColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ lazyLoadingOverlayColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
