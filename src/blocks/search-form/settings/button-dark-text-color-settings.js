...
export default function ButtonDarkTextColorSettings({
  attributes,
  setAttributes,
}) {
  const { buttonDarkTextColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonDarkTextColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Button text color (dark mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonDarkTextColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ buttonDarkTextColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
