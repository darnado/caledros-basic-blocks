...
export default function WidthSettings({ attributes, setAttributes }) {
  ...
  const [useCustomWidth, setUseCustomWidth] = useState(
    formWidth === "100%" ||
      formWidth === "var(--wp--style--global--content-size)" ||
      formWidth === "var(--wp--style--global--wide-size)"
      ? false
      : true
  );
  ...
  return (
    <PanelBody title={__("Width", "caledros-basic-blocks")} initialOpen={false}>
      <ToggleControl
      ...
      {!useCustomWidth && (
        <SelectControl
          ...
          onChange={(newWidth) => {
            setAttributes({ formWidth: newWidth });
          }}
        />
      )}
      {useCustomWidth && (
        <div className="cbb-editor__grid">
          <RangeControl
            ...
            step={1}
            onChange={(newWidth) => {
              setAttributes({ formWidth: `${newWidth}${widthUnit}` });
            }}
          />
          <SelectControl
            __nextHasNoMarginBottom
