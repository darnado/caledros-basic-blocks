...
export default function ButtonLightTextColorSettings({
  attributes,
  setAttributes,
}) {
  const { buttonLightTextColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonLightTextColor === "#00000000" ? true : false
  );
  ...
  return (
    <PanelBody
      title={__("Button text color (light mode)", "caledros-basic-blocks")}
      initialOpen={false}
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonLightTextColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ buttonLightTextColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
