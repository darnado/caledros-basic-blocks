...
export default function ButtonLightBackgroundColorSettings({
  attributes,
  setAttributes,
}) {
  const { buttonLightBackgroundColor } = attributes;
  const [useTransparentBackground, setUseTransparentBackground] = useState(
    buttonLightBackgroundColor === "#00000000" ? true : false
  );
    ...
    >
      <ToggleControl
        ...
        onChange={() => {
          ...
          setAttributes({ buttonLightBackgroundColor: "#00000000" });
        }}
      />
      {!useTransparentBackground && (
        <ColorPalette
          ...
          onChange={(newColor) =>
            setAttributes({ buttonLightBackgroundColor: newColor })
          }
          enableAlpha={true}
          clearable={false}
        />
